function countDown() {
  const diasCasamento = new Date('20 April 2024 16:00:00');
...
function sugerirMusica(event) {
  if (input.value.length > 0 || sugerir.value.length > 0) {
    const link = `https://api.whatsapp.com/send/?phone=5513997020701&text=Olá,+gostaria+de+sugerir+a+música+para+a+festa :+${
      input.value
...
document.querySelector('.copy-button').addEventListener('click', function () {
  var pixKey = 'welton.rodrigo@icloud.com'; // Substitua pelo valor da sua chave Pix
